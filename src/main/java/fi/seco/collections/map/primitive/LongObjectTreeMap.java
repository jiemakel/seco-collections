...
public class LongObjectTreeMap<T> implements ILongObjectTreeMap<T> {
	...
	private abstract class EntryIterator extends AIterableIterator<LongObjectCursor<T>> {
		Entry<T> next;
		Entry<T> current;

		protected LongObjectCursor<T> lc = new LongObjectCursor<T>();
	...
	public IIterableIterator<LongObjectCursor<T>> iterator() {
		return new EntryIterator(getFirstEntry()) {
			@Override
			public LongObjectCursor<T> next() {
				Entry<T> e = nextEntry();
				lc.key = e.getKey();
				lc.value = e.getValue();
				return lc;
	...
	@Override
	public final Entry<T> getCeilingEntry(long key) {
		Entry<T> p = root;
		while (p != null) {
			long cmp = compare(key, p.key);
			if (cmp < 0) {
				if (p.left != null)
					p = p.left;
				else return p;
	...
	@Override
	public final Entry<T> getFloorEntry(long key) {
		Entry<T> p = root;
		while (p != null) {
			long cmp = compare(key, p.key);
			if (cmp > 0) {
	...
	@Override
	public final Entry<T> getHigherEntry(long key) {
		Entry<T> p = root;
		while (p != null) {
			long cmp = compare(key, p.key);
			if (cmp < 0) {
	...
	@Override
	public final Entry<T> getLowerEntry(long key) {
		Entry<T> p = root;
		while (p != null) {
			long cmp = compare(key, p.key);
			if (cmp > 0) {
	...
	@Override
	public final Entry<T> getEntry(long key) {
		Entry<T> p = root;
		while (p != null) {
			long cmp = compare(key, p.key);
			if (cmp < 0)
	...
	private static final <T> long keyOrZero(Entry<T> e) {
		return e != null ? e.key : 0;
	}
	...
	private class SubMap implements ILongObjectTreeMap<T> {
		final ILongObjectTreeMap<T> m;

		final long lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;

		SubMap(ILongObjectTreeMap<T> m, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi,
				boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		final boolean tooLow(long key) {
			if (!fromStart) {
				long c = m.compare(key, lo);
				if (c < 0 || (c == 0 && !loInclusive)) return true;
			}
			return false;
		}

		final boolean tooHigh(long key) {
			if (!toEnd) {
				long c = m.compare(key, hi);
				if (c > 0 || (c == 0 && !hiInclusive)) return true;
			}
			return false;
		}

		final boolean inRange(long key) {
			return !tooLow(key) && !tooHigh(key);
		}

		final boolean inClosedRange(long key) {
			return (fromStart || m.compare(key, lo) >= 0) && (toEnd || m.compare(hi, key) >= 0);
		}

		final boolean inRange(long key, boolean inclusive) {
		...
		@Override
		public final long getCeilingKey(long key) {
			return keyOrZero(getCeilingEntry(key));
		}
		...
		@Override
		public final long getHigherKey(long key) {
			return keyOrZero(getHigherEntry(key));
		}
		...
		@Override
		public final long getFloorKey(long key) {
			return keyOrZero(getFloorEntry(key));
		}
		...
		@Override
		public final long getLowerKey(long key) {
			return keyOrZero(getLowerEntry(key));
		}
		...
		public long size() {
			...
			size = 0;
			Iterator<LongObjectCursor<T>> i = iterator();
			while (i.hasNext()) {
				size++;
				i.next();
			}
